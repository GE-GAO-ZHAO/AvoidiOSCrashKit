...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [NSObject talDefenderSwizzlingInstanceMethod:@selector(forwardingTargetForSelector:)
...
// 自定义实现 `-tal_forwardingTargetForSelector:` 方法
- (id)tal_forwardingTargetForSelector:(SEL)aSelector {
    ...
    return [self tal_forwardingTargetForSelector:aSelector];
}
...
+ (id)tal_forwardingTargetForSelector:(SEL)aSelector {
    ...
    if (!realize) {
        ...
        if (!realize) {
            ...
            // 如果类没有对应的方法，则动态添加一个
            if (!class_getClassMethod(NSClassFromString(className), aSelector)) {
                class_addMethod(cls, aSelector, (IMP)Crash, "@@:@");
            }
            // 把消息转发到当前动态生成类的实例对象上
            return [[cls alloc] init];
